...
public class complaintcode {
	Loadingfile bankfileR= new Loadingfile();
	...
	{
		for (HashMap.Entry<String,Complaintdetails> entry :bankfileR.newobj().entrySet()) 
		{
	...
	for (HashMap.Entry<String,Complaintdetails> entry :bankfileR.newobj().entrySet()) 
	{
		if(entry.getValue().complaintid.equals(id))//if(bankfileR.newobj().containsKey(id))
	  {
